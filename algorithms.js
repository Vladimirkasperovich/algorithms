...
const findMaxSalary = (data) => {
  const sortedSalary = [...data].sort((a, b) => a.salary - b.salary);
  const sortedNames = sortedSalary.map((item) => item.name);
  const maxSalary = sortedSalary[0].salary;

  const stats = {};
  sortedSalary.forEach(({ department, salary }) => {
    if (!stats[department]) {
      stats[department] = { totalSalary: 0, count: 0 };
    }
    stats[department].totalSalary += salary;
...
function groupUsersByGender(users) {
  // Ваш код здесь
  const group = {};
  users.forEach((item) => {
...
function countDuplicateCharacters(str) {
  ...
  for (const sElement of str.toLowerCase()) {
    seen[sElement] = (seen[sElement] || 0) + 1;
  }
  return Object.entries(seen).filter((item) => item[1] > 1).length;
}
